...
fn shape_dynamic_count<F: Field, V: Var<F>>(set: V, counts: V, frac: V, chall: V) -> V {
    frac * (set + chall) - counts
}

fn shape_fixed_count<F: Field, V: Var<F>>(set: V, frac: V, chall: V) -> V {
    ...
    V: Var<F>,
{
    ...
    //let chall = lookup_challenge.lookup_challenge();
    let chall = lookup_challenge;
    let left = shape_fixed_count(lookups, frac1, chall.clone());
